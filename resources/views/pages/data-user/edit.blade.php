        ...
        <form method="POST" action="{{ route('user.update', ['id' => $user->id, 'role' => $role]) }}">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="editUserRole" name="role" required>
                    <option value="admin" {{ old('role', $role) == 'admin' ? 'selected' : '' }}>{{ __('Admin')}}</option>
                    <option value="shift_leader" {{ old('role', $role) == 'shift_leader' ? 'selected' : '' }}>{{ __('Shift Leader')}}
                    </option>
                    <option value="employee" {{ old('role', $role) == 'employee' ? 'selected' : '' }}>{{ __('Employee')}}</option>
                </select>
            </div>
            ...
            @if (in_array(old('role', $role), ['shift_leader', 'employee']))
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="editUserGender" name="gender" required>
                        <option value="Pria" {{ old('gender', $user->gender) == 'Pria' ? 'selected' : '' }}>{{ __('Pria')}}</option>
                        <option value="Wanita" {{ old('gender', $user->gender) == 'Wanita' ? 'selected' : '' }}>{{ __('Wanita')}}
                        </option>
                    </select>
                </div>
            ...
            <div class="mb-3">
                <label for="editUserPassword" class="form-label">{{ __('Password')}}<small class="text-muted">{{ __('kosongkan jika tidak
                        diubah')}}</small></label>
            ...
            <a href="{{ route('data.user.index') }}" class="btn btn-secondary ms-2">{{ __('Batal')}}</a>
        </form>
    </div>
@endsection
